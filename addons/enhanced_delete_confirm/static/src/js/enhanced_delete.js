/** @odoo-module **/
import { registry } from "@web/core/registry";
import { listView } from "@web/views/list/list_view";
import { ListController } from "@web/views/list/list_controller";

export class EnhancedDeleteListController extends ListController {
    async onDeleteSelectedRecords() {
        const selectedRecords = this.model.root.selection;
        if (selectedRecords.length === 0) {
            return;
        }

        // Get record IDs and model name
        const recordIds = selectedRecords.map(record => record.resId);
        const modelName = this.model.root.resModel;

        try {
            // Call our wizard creation method
            const result = await this.orm.call(
                'enhanced.delete.confirm.wizard',
                'create_wizard',
                [recordIds, modelName],
                {}
            );

            if (result) {
                // Open the wizard
                const action = await this.orm.call(
                    'enhanced.delete.confirm.wizard',
                    'read',
                    [result, ['id']],
                    {}
                );

                return this.action.doAction({
                    type: 'ir.actions.act_window',
                    res_model: 'enhanced.delete.confirm.wizard',
                    res_id: result,
                    views: [[false, 'form']],
                    target: 'new',
                    context: {
                        'delete_record_ids': recordIds,
                        'delete_model': modelName,
                    },
                });
            }
        } catch (error) {
            console.log('Enhanced delete wizard failed, proceeding with normal delete:', error);
        }

        // Fallback to normal delete if wizard fails
        return super.onDeleteSelectedRecords();
    }
}

// Register the enhanced controller for list views
registry.category("views").add("list", {
    ...listView,
    Controller: EnhancedDeleteListController,
});

console.log("Enhanced Delete Confirm module loaded");